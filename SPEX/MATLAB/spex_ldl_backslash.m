...
%
% x may also be returned as a vpa matrix, or a cell array of strings, with x
% {i} = 'numerator/denominator', where the numerator and denominator are
% strings of decimal digits of arbitrary length.
%
% Usage:
%
% x = spex_ldl_backslash (A,b) returns the solution to Ax=b using default
%   settings.
%
% x = spex_ldl_backslash (A,b,options) returns the solution to Ax=b with user
...
% Example:
%
%   % In this first example, x = spex_ldl_backslash (A,b) returns an
%   % approximate solution, not because it was computed incorrectly.  It is
%   % computed exactly as a rational result with arbitrary precision, but then
%   % converted to double precision on output.
%
%   load west0479
%   A = west0479 ;
%   A = A'*A;
...
%   xtrue = rand (n,1) ;
%   b = A*xtrue ;
%   x = spex_ldl_backslash (A, b) ;
%   err = norm (x-xtrue)
%   x = A\b ;
%   err = norm (x-xtrue)
%
%   % In this example, x = spex_ldl_backslash (A,b) is returned exactly
...
%   resid = norm (A*x-b)
%
% See also vpa, spex_lu_backslash, spex_cholesky_backslash, spex_backslash,
%   spex_mex_install, spex_mex_test, spex_mex_demo

% spex_ldl_backslash is a wrapper for the exact routines contained within the
% SPEX software package.  In order to use spex_ldl_backslash you must install
...
% Preprocessing complete. Now use SPEX ldl to solve A*x=b.
x=spex_ldl_mex_soln (A, b, option) ;
