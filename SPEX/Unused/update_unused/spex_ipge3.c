...
SPEX_info spex_ipge3 // perform IPGE on x based on v
(
    ...
    mpq_t x_scale, // the first pending scale for v
    mpq_t v_scale, // the first pending scale for v
...
)
{
    ...
    if (real_hj < j-1) // require history update
    {
        SPEX_CHECK(SPEX_mpz_mul(sv_x->x[perm_j], sv_x->x[perm_j],
                                sd[j-1]));
        if (real_hj > -1)
        {
    ...
    SPEX_CHECK(SPEX_mpq_div(pending_scale, x_scale,v_scale));
    //SPEX_CHECK(SPEX_mpq_set(pending_scale, v_scale));
    if (j > 0)
    {
        SPEX_CHECK(SPEX_mpz_mul(SPEX_MPQ_NUM(pending_scale),
                                SPEX_MPQ_NUM(pending_scale), sd[j-1]));
    ...
    for (p = 1; p < v->nz; p++)
    {
        ...
        if (sgn != 0)    // x[i] != 0
        {
            if (real_hi < j-1)
            {
                ...
                if (real_hi > -1)
                {
                    SPEX_CHECK(SPEX_mpz_divexact(sv_x->x[i], sv_x->x[i],
                                        sd[real_hi]));
                }
            }
            // x[i] = x[i]*v[perm[j]]
        ...
        // -----------------------------------------------------------------
        // x[i] = (x[i]-v[i]*x[perm[j]])/M->v[j-1]->x[0].
        // -----------------------------------------------------------------
    ...
    if(mpz_cmp_ui(SPEX_MPQ_DEN(pending_scale), 1) != 0 )
    {
        printf("%ld-th IPGE\n",j);
    ...
    //gmp_printf("%Qd\n",pending_scale);
    SPEX_CHECK(SPEX_mpz_mul(sv_x->x[perm_j], sv_x->x[perm_j],
                            SPEX_MPQ_DEN(x_scale)));
    SPEX_CHECK(SPEX_mpz_set(SPEX_MPQ_DEN(x_scale), SPEX_MPQ_DEN(pending_scale)));
