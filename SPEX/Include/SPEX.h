...
// SPEX: (c) 2019-2026, Christopher Lourenco, Jinhao Chen,
...
#define SPEX_DEFAULT 0
    ...
    // NOTE: The SPEX_vector object will be used in a future version of SPEX.
    ...
    // Not all functions accept all 15 matrices types, however.
...
#define SPEX_FREE(p)  \
    ...
    // factorization. A must be square and can be full rank or rank deficient
    //------------------------------------------------------------------------------
    ...
    // This portion of the SPEX library exactly solves sparse symmetric positive
    // definite (SPD) or symmetric negative definite (SND) linear systems.
    // The code accompanies the paper published in ACM TOMS
    ...
    //  "Algorithm 1XXX: SPEX QR and SPEX Rank for Exactly Revealing Rank and
    ...
    //              http://www.mpfr.org/
    //
    //   SPEX_Utilities, AMD, and COLAMD are distributed along with SPEX
    //   The easiest way ensure these dependencies are met is to only access this
    //   package through the SPEX repository.
    //
    //   All of these codes are components of the SPEX software library. This code
    ...
    //  1) Solve a linear system using an appropriate method. If A is rectangular,
    //     an exact QR factorization is used. Otherwise, if A is symmetric, an
    //     exact LDL factorization is used. If A is square and not symmetric, or
    //     if the LDL factorization fails, an exact LU factorization is used.
    //  2) Compute the rank of A. If A is square a rank revealing LU factorization
    //     is used. If A is rectangular, a rank revealing QR factorization is used.

    // For either method A and b may be stored as either int64, double precision,
