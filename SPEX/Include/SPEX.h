...
#define SPEX_DATE "May FIXME, 2025"
#define SPEX_VERSION_STRING "3.3.0"
...
typedef enum
{
    ...
    SPEX_INCONSISTENT = -9,       // for QR factorization (FUTURE)
}
SPEX_info ;
...
// SPEX_factorization_algorithm is an enum in SPEX_options to tell SPEX which
// factorization algorithm to use.  By default, left-looking is used for LU and
// up-looking is used for Cholesky/LDL. If desired, left-looking can be used
// for all Cholesky or LDL routines by setting option->algo to SPEX_CHOL_LEFT
// or SPEX_LDL_LEFT, respectively.  Witin SPEX Backslash, option->algo utilizes
// the specified algorithm if option->algo is set. Otherwise, if option->algo
// is left as default, SPEX Backslash attempts an LDL factorization and, if
// that fails, an LU factorization is done.
...
// A SPEX_vector is a sparse vector used for dynamic (updatable) SPEX
// matrices.
...
// initialized, and *v_handle->i is allocated with length of nzmax.
...
// Purpose: This function realloc a SPEX_vector to given new size. If
// new_size > (v->nzmax), it will reallocate v->i and v->x with new size, and
// initialize/allocate for all the new mpz entries. Otherwise, It will first
...
// The SPEX_DYNAMIC_CSC format only supports the mpz_t type, while each of
...
//     independently and dynamically; only used in the update algorithms.
...
//     A->v[j]->nz-1] give the row indices of all nonzeros, and the mpz_t
//     values of these entries appear in the same locations in A->v[j]->x.
...
typedef struct
{
    ...
    SPEX_type type ;    // mpz, mpq, mpfr, int64, or fp64 (double)
                        // NOTE: entries of SPEX_DYNAMIC_CSC matrix must be
                        // mpz type.
...
// The SPEX_factorization object holds an LU, Cholesky/LDL, or (a future) QR
// numerical factorization, in either non-updatable (static) or updatable form.

// All SPEX_update_* functions require an updatable factorization with
...
// the way that L (and U if exists) is stored. User can get more details of the
// difference between updatable and non-updatable forms by referring to the
// comments for SPEX_factorization_convert in this file.
...
// SPEX_cholesky_factorize, or SPEX_ldl_factorize. All these function will
// create a static factorization of corresponding kind.

// To convert between updatable and non-updatable (static) factorization, user
// should call SPEX_factorization_convert, which performs in-place
// bi-directional conversion. In addition, all functions that require the
// updatable format check the input factorization and convert it automatically.
// If this happens, the output factorization becomes updatable.
//
...
// the corresponding SPEX_update_lu_colrep or SPEX_update_symmetric_rank1.
...
typedef struct
{
    ...
    bool updatable;                       // flag to denote if the factorization
                                          // is in the updatable format.  Only
                                          // LU, Cholesky, and LDL can be
                                          // in updatable format.
    ...
    // For kind == SPEX_LU_FACTORIZATION, Qinv_perm can be NULL; it is only
    // constructed if the LU factorization is to be updated by
    // SPEX_update_lu_colrep.
...
//------------------------------------------------------------------------------
// For LU, LDL, or Cholesky factorization, if user wishes to perform
// factorization update, then it must be in updatable format (i.e.,
// F->updatable == true).  This requires all the following conditions to be
// met.
//
// 1. Both L and U are stored as SPEX_DYNAMIC_CSC. Notably, U in the updatable
//    factorization is actually the transpose of U (or equivalently, U is stored
//    by row instead), since U will be updated one row at a time.
//
// 2. A = LD^(-1)U, which means L and U are properly permuted. (Recall that
//    PAQ = LDU or PAP' = LDL' holds for static factorization). That is, for
//    updatable factorization, the rows of L are in the same order as the rows
//    of A, while the j-th column of L (F->L->v[j]) contains the j-th pivot,
//    which would be F->L->v[j]->x[0], (i.e., F->L->v[j]->i[0] ==
//    F->P_perm[j]); the columns of U (or the rows of UT) are in the same order
//    as the columns of A, while the j-th row of U (or the j-th column of UT)
//    (F->U->v[j]) contains the j-th pivot, which would be F->U->v[j]->x[0],
//    (i.e., F->U->v[j]->i[0] == F->Q_perm[j]).
//
...
// claim it is updatable, and vice versa. To correctly convert the
// factorization, user should call SPEX_factorization_convert to convert the
// given F to either updatable or non-updatable as specified by the boolean
// input argument 'updatable'. If F->updatable == updatable holds upon input,
// this function does nothing. Otherwise, it performs the corresponding
// in-place conversion.
//
// NOTE:
// Upon input, F->L (and F->U if exists) must be non-shallow CSC MPZ matrix for
// non-updatable (static) factorization (i.e., F->updatalbe == false),
// otherwise, SPEX_INCORRECT_INPUT will be returned. Likewise, F->L (and F->U
// if exists) must be dynamic_CSC MPZ matrix for updatable factorization. All
...
// do not try to modify any individual component of F.
//
...
SPEX_info SPEX_mpz_add (mpz_t x, const mpz_t y, const mpz_t z) ;
...
SPEX_info SPEX_mpz_mul (mpz_t x, const mpz_t y, const mpz_t z) ;
SPEX_info SPEX_mpz_mul_si (mpz_t x, const mpz_t y, const int64_t z) ;
...
//    The SPEX LU routines solve the linear system Ax = b exactly. The input
...
//
// This is just a caller to the original Cholesky solve...
// If we have LDL' x = b then the transpose solve is also LDL' x = b

SPEX_info SPEX_cholesky_tsolve
(
    // Output
    SPEX_matrix *x_handle,      // On input: undefined.
                                // On output: Rational solution (SPEX_MPQ)
...
// The SPEX Cholesky:Rank1 routines in the SPEX library update a exact Cholesky
// factorization PAQ = LDL^T exactly when A is changed as A' = A+sigma*w*w^T.

// The SPEX LU:ColRep routine update an exact LU factorization PAQ = LDU when A
// is changed with a single column.
...
// This function performs LU update for column replacement. The input
...
//------------------------------------------------------------------------------
// Symmetric Rank-1 update/downdate for Cholesky and LDL factorizations
//------------------------------------------------------------------------------

// This function performs symmetric rank-1 update/downdate of a Cholesky or LDL
// factorization. The input factorization needs to be updatable with L being
// SPEX_DYNAMIC_CSC MPZ matrix. Otherwise (if F is non-updatable upon input),
// this function calls SPEX_factorization_convert to make F updatable, which
// requires that L in the factorization must be non-shallow SPEX_CSC MPZ
// matrix. The output factorization will always be updatable.  Since the
// factorization is modified during the update process, the returned F should
// be considered as undefined if this function fails for any reason.
...
SPEX_info SPEX_tsolve       // solves the transposed system A'x = b
(
    // Output
    SPEX_matrix *x_handle,  // rational solution to the system
    // input/output:
