...
(
    ...
    int64_t **p_handle,        // row permutation
    // input:
    const SPEX_matrix *A,      // matrix to be factored
    const int64_t *S_q,        // column permutation
    const int64_t S_lnz,       // estimate of nnz in L
    const int64_t S_unz,       // estimate of nnz in U
...
)
{
    ...
    // memory usage.
    SPEX_CHECK (SPEX_matrix_allocate(&L, SPEX_CSC, SPEX_MPZ, n, n, S_lnz,
        false, false, option));
    SPEX_CHECK (SPEX_matrix_allocate(&U, SPEX_CSC, SPEX_MPZ, n, n, S_unz,
        false, false, option));
