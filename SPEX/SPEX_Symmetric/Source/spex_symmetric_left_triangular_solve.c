//------------------------------------------------------------------------------
// SPEX_Symmetric/spex_symmetric_left_triangular_solve: sparse symmetric
...
)
{
    ...
    // L(k,0:k-1). Note that the left-looking Cholesky/LDL factorization
    // performs two elimination tree analyses. The first is done prior to here
    // in the preallocation of the L matrix. The second, performed here, gets
    // the nonzero pattern of L(k,:) (To compute L(:,k) you need the
    // prealocation first).
    ...
    // compute L(:,k) -= L(k+1:n,1:k-1) * D(1:k-1,1:k-1) * L(k,1:k-1)' but also
    ...
    for (p = top ; p < col_top ; p++)
    {
        ...
        // update kth column of L with the jth column of L
        ASSERT (j < k) ;
    ...
    //--------------------------------------------------------------------------
    // finalize the kth column of L by finalizing the history updates
    //--------------------------------------------------------------------------
