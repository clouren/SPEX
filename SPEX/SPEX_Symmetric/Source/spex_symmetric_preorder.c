//------------------------------------------------------------------------------
// SPEX_Symmetric/spex_symmetric_preorder: symbolic ordering/analysis for
// Cholesky/LDL
// ------------------------------------------------------------------------------
 ...
 *
 * This function performs the symbolic ordering for SPEX Cholesky or SPEX LDL.
 * Currently, there are three options: user-defined order, COLAMD, or AMD.  It
 * is *highly* recommended that AMD is used for symmetric (Cholesky or LDL)
 * factorization.
 *
...
)
{
    ...
    // m = n for Cholesky/LDL factorization
    ASSERT(A->n == A->m);
    ...
    switch(order)
    {
        ...
        case SPEX_AMD:
        // ---AMD ordering is used (DEFAULT)--- S->p is set to AMD's symmetric
        // ordering on A.  The number of nonzeros in L is given as AMD's
        // computed number of nonzeros in the Cholesky/LDL factor L of A which
        // is the exact nnz(L) for Cholesky/LDL factorization (barring numeric
        // cancellation)
        {
