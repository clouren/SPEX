//------------------------------------------------------------------------------
// SPEX_Backslash/SPEX_tsolve.c: Given a factorization, solve a transposed system A'x=b
//------------------------------------------------------------------------------
 ...
 *          Importantly, note that this is to solve the transposed linear system
 *          A'x = b
 *
 * Input/Output arguments:
 *
 * x_handle:    Pointer to the exact solution of the system A'x = b
 *
...
(
    ...
    SPEX_matrix *x_handle,      // On input: undefined.
                                ...
                                // to the system.
    // input/output:
    SPEX_factorization F,       // The Cholesky, LU or LDL factorization of A
    // input:
...
)
{
    ...
    switch (f_kind)
    {
        ...
        case SPEX_LU_FACTORIZATION:
            info = SPEX_lu_tsolve(&x, F, b, option);
            break ;
