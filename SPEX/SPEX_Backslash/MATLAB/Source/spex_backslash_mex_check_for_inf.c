// //------------------------------------------------------------------------------
// SPEX_Backslash/MATLAB/spex_backslash_mex_check_for_inf: Check A and b for inf
//------------------------------------------------------------------------------
...
bool spex_backslash_mex_check_for_inf     // true if x can be represented as int64_t
(
...
)
{
    ...
    for (mwSize k = 0; k < n; k++)
    {
        ...
        if (mxIsInf (xk))
        {
            spex_backslash_mex_error (1, "A must not have any Inf values") ;
        }

        if (mxIsNaN (xk))
        {
            spex_backslash_mex_error (1, "A must not have any NaN values") ;
        }
