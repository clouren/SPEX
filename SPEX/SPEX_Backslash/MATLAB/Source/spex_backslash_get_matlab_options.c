//------------------------------------------------------------------------------
// SPEX_Backslash/MATLAB/spex_get_matlab_options: Set factorization options for SPEX LU
//------------------------------------------------------------------------------
...
void spex_backslash_get_matlab_options
(
...
)
{
    ...
    if (field != NULL)
    {
        if (!mxIsChar (field)) spex_backslash_mex_error (1, "option.order must be a string") ;
        ...
        {
            spex_backslash_mex_error (1, "unknown option.order") ;
        }
    }
    ...
    if (field != NULL)
    {
        ...
        {
            spex_backslash_mex_error (1, "unknown option.solution") ;
        }
    }
    ...
    if (field != NULL)
    {
        ...
        if (d != trunc (d) || d < 2 || d > (1 << 29))
        {
            ...
            spex_backslash_mex_error (1, "options.digits must be an integer "
                "between 2 and 2^29") ;
        }
