//------------------------------------------------------------------------------
// SPEX_Backslash/MATLAB/spex_backslash_mex_get_A_and_b: Obtain user's A and b matrices
//------------------------------------------------------------------------------
...
void spex_backslash_mex_get_A_and_b
(
...
)
{
    ...
    if (!A_handle || !pargin)
    {
        spex_backslash_mex_error (SPEX_INCORRECT_INPUT, "");
    }
    (*A_handle) = NULL ;
    ...
    if (nA != mA)
    {
        spex_backslash_mex_error (1, "A must be square") ;
    }

    // check the values of A
    bool A_has_int64_values = spex_backslash_mex_check_for_inf (Ax, Anz) ;
    ...
    if (mb != mA)
    {
        spex_backslash_mex_error (1, "dimension mismatch") ;
    }

    int64_t count = 0;

    // check the values of b
    bool b_has_int64_values = spex_backslash_mex_check_for_inf (bx, nb*mb) ;
