//------------------------------------------------------------------------------
// SPEX_Backslash/MATLAB/SPEX_Backslash_mex_soln: Use SPEX Backslash within MATLAB
//------------------------------------------------------------------------------
...
{
    //--------------------------------------------------------------------------
    // Initialize SPEX Backslash library environment
    //--------------------------------------------------------------------------
    ...
    {
        spex_backslash_mex_error (1, "Usage: x = SPEX_backslash_soln (A,b,option)");
    }

    //--------------------------------------------------------------------------
    // check inputs for for reals and sparsity
    //--------------------------------------------------------------------------

    if (mxIsComplex(pargin [0]) || mxIsComplex(pargin [1]))
    {
        spex_backslash_mex_error(1, "inputs must be real");
    }
    /***/
    if (!mxIsSparse(pargin [0]))     // Is the matrix sparse?
    {
        spex_backslash_mex_error(1, "first input must be sparse");
    }
    /**/
    if (mxIsSparse(pargin [1]))         // Is b sparse?
    {
        spex_backslash_mex_error(1, "second input must be dense");
    }
    ...
    if (option == NULL)
    {
        spex_backslash_mex_error (SPEX_OUT_OF_MEMORY, "");
    }

    spex_mex_options mexoptions;
    if (nargin > 2) 
    {
        spex_backslash_get_matlab_options (option, &mexoptions, pargin [2]);
    }
    ...
    SPEX_matrix *A = NULL ;
    SPEX_matrix *b = NULL ;
    spex_backslash_mex_get_A_and_b (&A, &b, pargin, option);

    // Print the matrices if user specified a print level
    ...
    else
    {
        ...
        for (int64_t p = 0; p < mn; p++)
        {
            ...
            if (status < 0)
            {
                spex_backslash_mex_error(1, "error converting x to string");
            }
