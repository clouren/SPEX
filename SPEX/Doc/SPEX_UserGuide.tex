...
\vspace{5mm}
Version 1.1.1, February 2021 % VERSION
\vspace{20mm}
...
SPEX is a software package for SParse EXact linear algebra routines. It currently comprises of the following:
	\begin{itemize}
	\item SPEX Utilities: Utility functions for all SPEX subroutines. Primarily comprises of our interface to the GMP/MPFR library, all memory management functions, our \verb|SPEX_matrix| data structure, and various factorization utility functions. Please refer to Chapter \ref{ch:Util} for further details.
	\item SPEX Left LU: Sparse exact LU factorization. Using a bit-complexity model, solves the linear system $A \mathbf{x} = \mathbf{b}$ in time proportional to arithmetic work which is a theoretically perfect complexity bound. Please refer to Chapter \ref{ch:LeftLU} for further details.
	\end{itemize}

\noindent \textbf{Copyright:} The SPEX software package is copyright by Chris Lourenco and all SPEX coauthors: Jinhao Chen, Tim Davis and Erick Moreno-Centeno. All rights reserved.

\noindent \textbf{Contact Info:} Contact Chris Lourenco,
\href{mailto:chrisjlourenco@gmail.com}{chrisjlourenco@gmail.com}, 
\href{mailto:lourenco@usna.edu}{lourenco@usna.edu}
or Tim Davis,
\href{mailto:timdavis@aldenmath.com}{timdavis@aldenmath.com},
...
version 3. Details of this license are in \verb|SPEX/License/license.txt|.
...
\noindent \textbf{Required Packages:} SPEX requires the installation of AMD
...
SPEX Util is a software package containing utility functions for all other SPEX 
...
libraries in order to prevent memory leaks and improve the overall stability of
these external libraries. SPEX Util is written in ANSI C.

SPEX operates on matrices stored in any of the following 15 combinations: 
$\{$Compressed Sparse Column (CSC), triplet, dense$\} \times \{$ \verb|mpz_t|,
\verb|mpq_t|, \verb|mpfr_t|, \verb|int64_t|, or \verb|double|$\}$. Most routines
assume that the matrix is in CSC form with \verb|mpz_t| (i.e., big integer) data
types. Using the SPEX matrix copy function, any matrix can be copied and converted
...
see Section~\ref{ss:SPEX_matrix}.
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_info|: status code returned by SPEX}
\label{ss:SPEX_info}
%-------------------------------------------------------------------------------
...
object is returned.  Returns \verb|ok| as \verb|false| if SPEX has not been
initialized.
...
%-------------------------------------------------------------------------------
\cprotect\section{The \verb|SPEX_options| object:
parameter settings for SPEX} \label{ss:SPEX_options}
%-------------------------------------------------------------------------------

The \verb|SPEX_options| object contains numerous parameters that may be
...
functions create and destroy a \verb|SPEX_options| object:
...
\hline
\verb|SPEX_FREE|
    & destroy \verb|SPEX_options| object
    & \ref{ss:SPEX_free} \\
\hline
\end{tabular}
\end{center}
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_create_default_options|: create default \verb|SPEX_options| object}
...
%-------------------------------------------------------------------------------
\cprotect\section{The \verb|SPEX_matrix| object} \label{ss:SPEX_matrix}
%-------------------------------------------------------------------------------

All matrices for SPEX are stored as a \verb|SPEX_matrix| object (a pointer
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_type|: enum for data types of matrix entry}
\label{ss:SPEX_type}
%-------------------------------------------------------------------------------
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_matrix_allocate|: allocate a $m$-by-$n$
\verb|SPEX_matrix|}
...
%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_matrix_copy|: make a copy of a \verb|SPEX_matrix|}
...
\verb|SPEX_matrix_nnz| returns the number of entries in a \verb|SPEX_matrix *A|.
...
Returns -1 if SPEX LU has not been initialized.

%-------------------------------------------------------------------------------
\cprotect\subsection{\verb|SPEX_matrix_check|: check and print a \verb|SPEX_matrix|}
...
\verb|SPEX_LU_analyze|
    & create \verb|SPEX_LU_analysis| object
    & \ref{s:SPEX_LU_analyze} \\
\hline
\verb|SPEX_LU_analysis_free|
    & destroy \verb|SPEX_LU_analysis| object
...
guaranteed to be exact. In addition, SPEX Left LU provides a wrapper class for
the GNU Multiple Precision Arithmetic (GMP) \cite{granlund2015gnu} and GNU
Multiple Precision Floating Point Reliable (MPFR) \cite{fousse2007mpfr}
libraries in order to prevent memory leaks and improve the overall stability of
these external libraries. SPEX Left LU is written in ANSI C and is accompanied
by a MATLAB interface.
...
matrix formats and data types in the \verb|SPEX_matrix|, and to
...
One key advantage of utilizing SPEX Left LU with floating-point output is that
the solution is guaranteed to be exact until this final conversion; meaning
that roundoff errors are only introduced in the final conversion from rational
numbers. Thus, the solution $x$ output in \verb|double| precision are accurate
to machine roundoff (approximately $10^{-16}$) and SPEX Left LU utilizes higher
...
\noindent \textbf{Required Packages:} SPEX Left LU requires the installation of AMD
\cite{amestoy1996approximate,amestoy2004algorithmamd}, COLAMD
\cite{davis2004column,davis2004algorithmcolamd}, \verb|SuiteSparse_config|
\cite{davis2020suitesparse}, the GNU GMP \cite{granlund2015gnu} and GNU MPFR
\cite{fousse2007mpfr} libraries.  AMD and COLAMD are available under a BSD
...
If you want to use SPEX Left LU within MATLAB, from your installation of MATLAB,
\verb|cd| to the folder \verb|SPEX/SPEX/SPEX_Left_LU/MATLAB| then type
\newline \verb|SPEX_Left_LU_install|. This should compile the necessary code so that you can use
the \verb|SPEX_Left_LU_backslash| function from within MATLAB. Note that
...
\section{Left LU Factorization and Solve Routines}
...
For additional information, refer to either \verb|SPEX/SPEX/SPEX_Left_LU/Demos/spexlu_denmo.c| or
...
%-------------------------------------------------------------------------------
\cprotect\subsection{Simple SPEX LU routines for solving linear systems}
...
%-------------------------------------------------------------------------------
\cprotect\subsection{Expert SPEX LU routines}
\label{s:Using:expert}
%-------------------------------------------------------------------------------

If the $L$ and $U$ factors from the SPEX LU factorization of the matrix $A$
...
Using SPEX in this form requires the intermediate variables be declared,
...
%-------------------------------------------------------------------------------
\subsubsection{SPEX LU symbolic analysis}
%-------------------------------------------------------------------------------

The symbolic analysis phase of SPEX LU computes the column permutation and
...
of functions/macros to free SPEX objects:
...
%-------------------------------------------------------------------------------
\cprotect\subsection{Examples of using SPEX LU in a C program}
...
%-------------------------------------------------------------------------------
\cprotect\section{Using SPEX in MATLAB}
\label{s:Use:MATLAB}
%-------------------------------------------------------------------------------

After following the installation steps discussed in Section \ref{s:install},
using the SPEX LU factorization within MATLAB can be done via the
...
The SPEX LU MATLAB interface includes an \verb|option| struct as in optional
...
The SPEX LU function expects as input a sparse matrix $A$ and dense set of
