...
int main( int argc, char* argv[])
{
    ...
    while (glp_get_status(LP) != GLP_OPT)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (j < n)
            {
               /* int rj_nz = glp_get_mat_row(LP, j+1, row_ind, NULL);
                printf("num of nz in row %ld is %d\n", j,rj_nz);
#if 1
                if (rj_nz > 0)
                {
                    int usable_col_count = 0, use_this_col = 0;
                    for (k = 1; k <= rj_nz; k++)
                    {
                        printf("%d: %d",row_ind[k], glp_get_col_bind(LP,row_ind[k]));
                        if (glp_get_col_bind(LP, row_ind[k]) == 0)
                        {
                            printf(" usable");
                            use_this_col = row_ind[k];
                            usable_col_count++;
                        }
                        printf("\n");
                    }
                    if (usable_col_count == 1)
                    {
                        glp_set_row_stat(LP, j+1, GLP_NL);
                        glp_set_col_stat(LP, use_this_col, GLP_BS);
                        one_more_simplex = true;
                        num_of_new_basis++;
                        break;
                    }
                    else if (usable_col_count > 1)
                    {
                        one_more_simplex = true;
                        break;
                    }
                ...
                if (rj_nz > 0)
                {
                    int usable_col_count = 0;
                    bool bf_exists = true;
                    int s1 = glp_get_row_stat(LP, j+1);
                    for (k = 1; k <= rj_nz; k++)
                    {
                        printf("%d: %d",row_ind[k], glp_get_col_bind(LP,row_ind[k]));
                        if (glp_get_col_bind(LP, row_ind[k]) == 0)
                        {
                            printf(" usable");
                            usable_col_count++;
                            int s2 = glp_get_col_stat(LP, row_ind[k]);
                            glp_set_col_stat(LP, row_ind[k], GLP_BS);
                            glp_set_row_stat(LP, j+1, GLP_NL);
                            bf_exists = glp_bf_exists(LP);
    printf("line %d\n",__LINE__);
                            if (bf_exists)
                            {
                                num_of_new_basis++;
                            ...
                            {
                                glp_set_row_stat(LP, j+1, s1);
                                glp_set_col_stat(LP, row_ind[k], s2);
    glp_factorize(LP); // in order to use glp_get_bhead
                            }
                        }
                        printf("\n");
                    }
                    if (!bf_exists || usable_col_count == 0)
                    {
                        glp_set_row_stat(LP, j+1, s1);
                    }
                    else //if (usable_col_count > 0 && bf_exists)
                    {
                        one_more_simplex = true;
                        break;
                    }
