...
)
{
    ...
    for (int64_t j = 0; j < remain_vars; j++)
    {
        ...
        if (lb == -1e308)
        {
            printf("%ld: lb=%lf(=-inf) ub=%lf coef=%lf\n", j, lb, ub,
                MY_MAT_X(Mc, j));
        }
...
)
{
    ...
    // -------------------------------------------------------------------------
    // create the LP for CPLEX
    // -------------------------------------------------------------------------
    double *coefs = (double*) SPEX_malloc(nvars*sizeof(double));
    double *lb = (double*) SPEX_malloc(nvars*sizeof(double));
    double *ub = (double*) SPEX_malloc(nvars*sizeof(double));
    double *matval = (double*) SPEX_malloc(nz*sizeof(double));
    double *rhs = (double*) SPEX_malloc(neqs*sizeof(double));
    char *sense = (char*) SPEX_malloc(neqs * sizeof(char));
    int *matcnt = (int*) SPEX_malloc(nvars * sizeof(int));
    int *matbeg = (int*) SPEX_malloc(nvars * sizeof(int));
    int *matind = (int*) SPEX_malloc(nz    * sizeof(int));
    for (int i = 0; i < neqs; i++)
    {
        sense[i] = 'E';
        rhs[i] = (double)(MY_MAT_X(Mb, i));
    }
    for (int j = 0; j < nvars; j++)
    {
        coefs [j] = (double) (MY_MAT_X(Mc, j));
        lb    [j] = (double) (MY_MAT_X(Mlb,j));
        ub    [j] = (double) (MY_MAT_X(Mub,j));
        matcnt[j] = (int) (MA_CSC->p[j+1] - MA_CSC->p[j]);
        matbeg[j] = (int) (MA_CSC->p[j]);
        for (int p = MA_CSC->p[j]; p < MA_CSC->p[j+1]; p++)
        {
            matind[p] = (int) (MA_CSC->i[p]);
            matval[p] = (double) (MY_MAT_X(MA_CSC,p));
        }
    }
    *LP = CPXcreateprob(env, status, "lp");
    ...
    printf("SPEX_FP64=%d %d %d %d %d %d\n",SPEX_FP64,Mc->type,Mb->type,MA_CSC->type,Mlb->type,Mub->type);
   *status = CPXsetintparam (env, CPXPARAM_Read_DataCheck,
			    CPX_DATACHECK_ASSIST);
   if (*status) {fprintf (stderr, "set para error %d.\n", *status);}

    *status = CPXcopylp (env, *LP, nvars, neqs, CPX_MIN,
        coefs, rhs, sense, matbeg, matcnt, matind, matval, lb, ub, NULL);
   if (*status) {fprintf (stderr, "build LP error %d.\n", *status);}

    int probtype = CPXgetprobtype(env, *LP);
    printf("prob type %d\n", probtype);

    *status = CPXgetobj(env, *LP, coefs, 0, nvars-1);
   if (*status) {fprintf (stderr, "coef error %d.\n", *status);}
    *status = CPXgetrhs(env, *LP, rhs,  0, neqs-1);
   if (*status) {fprintf (stderr, "rhs error %d.\n", *status);}
    *status = CPXgetlb(env, *LP, lb, 0, nvars-1);
   if (*status) {fprintf (stderr, "lb error %d.\n", *status);}
    *status = CPXgetub(env, *LP, ub, 0, nvars-1);
   if (*status) {fprintf (stderr, "ub error %d.\n", *status);}
    for (int i = 0; i < neqs; i++)
    {
        if (rhs[i] != MY_MAT_X(Mb,i))
        {
            printf("     rhs[%d] LP=%f real=%f\n",i,rhs[i],MY_MAT_X(Mb,i));
        }
    }
    for (int j = 0; j < nvars; j++)
    {
        for (int p = MA_CSC->p[j]; p < MA_CSC->p[j+1]; p++)
        {
            int i = MA_CSC->i[p];
            double Aij = 0;
            *status = CPXgetcoef(env, *LP, i, j, &Aij);
   if (*status) {fprintf (stderr, "Aij error %d.\n", *status);}
            if (Aij != MY_MAT_X(MA_CSC, p))
            {
                printf("A[%d,%d] LP=%f real=%f\n",i,j,Aij,MY_MAT_X(MA,p));
            }
        }
        if (coefs[j] != MY_MAT_X(Mc,j))
        {
            printf("c[%d] LP=%f real=%f\n",j,coefs[j],MY_MAT_X(Mc,j));
        }
        if (lb[j] != MY_MAT_X(Mlb,j))
        {
            printf("lb[%d] LP=%f real=%f\n",j,lb[j],MY_MAT_X(Mlb,j));
        }
        if (ub[j] != MY_MAT_X(Mub,j) &&
            !(ub[j] == CPX_INFBOUND && MY_MAT_X(Mub, j) == 1e308))
        {
            printf("ub[%d] LP=%f real=%f\n",j,ub[j],MY_MAT_X(Mub,j));
        }
    }
