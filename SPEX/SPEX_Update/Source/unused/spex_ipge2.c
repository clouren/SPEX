...
SPEX_info spex_ipge2 // perform IPGE on x based on v
(
...
)
{
    ...
    if (real_hj > -1)
    {
        SPEX_CHECK(SPEX_mpq_set_den(pending_scale, M->v[real_hj]->x[0]));
        ...
        if (real_hi != real_hj)
        {
            ...
            if (real_hi > -1)
            {
                mpz_fdiv_qr(sv_x->x[i], SPEX_MPQ_NUM(tmpq2),
                            sv_x->x[i], M->v[real_hi]->x[0]);
                SPEX_CHECK(SPEX_mpq_set_den(tmpq2,M->v[real_hi]->x[0]));
    ...
    {
        SPEX_CHECK(SPEX_mpz_mul(sv_x->x[perm_j], SPEX_MPQ_NUM(pending_scale),
                                M->v[j-1]->x[0]));
