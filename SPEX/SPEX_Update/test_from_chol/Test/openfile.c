...
int main( int argc, char* argv[])
{
    ...
    sprintf(file_name, "Results/LPnetlib/%s_chol50.txt",prob_name);
    mat_file = fopen(file_name, "w");
    if (mat_file == NULL)
    {
        printf("Could not open file\n");
        return 0;
    ...
    while (iter < 100) //(1)
    {
        ...
        fprintf(mat_file, "%lf \t%lf ",t_solve,
            (double) (end1 - start1) / CLOCKS_PER_SEC);
...
//        if (info == SPEX_OK) {printf("matrix is not singular!\n");}
        ...
        fprintf(mat_file, "\t%lf \t%ld   \t%ld    \t%ld   \t%ld ", t_llu,
            L1->p[n], L1_sum_size, U1->p[n], U1_sum_size);
        if (iter > 0)
        {
            printf("\n\"LUU_lb\": %lf \t%ld   \t%ld    \t%ld   \t%ld", t_luu3,
                L3_nnz, L3_sum_size, U3_nnz, U3_sum_size);
            fprintf(mat_file, "\t%lf \t%ld   \t%ld    \t%ld   \t%ld ", t_luu3,
                L3_nnz, L3_sum_size, U3_nnz, U3_sum_size);
        }
        else
        {
            fprintf(mat_file, "\t%d \t%d   \t%d    \t%d  \t%d ", 0,
                0, 0, 0, 0);
        }
        printf("\nLUU: \t%lf \t%ld   \t%ld    \t%ld   \t%ld", t_luu,
            L_nnz, L2_sum_size, U_nnz, U2_sum_size);
        fprintf(mat_file, "\t%lf \t%ld   \t%ld    \t%ld   \t%ld\n", t_luu,
            L_nnz, L2_sum_size, U_nnz, U2_sum_size);
