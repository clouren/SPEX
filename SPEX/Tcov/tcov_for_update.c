    ...
    for (n = n_min; n < n_max; n++)
    {
        ...
        GOTCHA;
            ...
            if (read_matrix)
            {
                ...
                TEST_CHECK(SPEX_matrix_allocate(&L, SPEX_DYNAMIC_CSC, SPEX_MPZ,
                    An, An, 0, false, true, option));
                TEST_CHECK(SPEX_matrix_allocate(&U, SPEX_DYNAMIC_CSC, SPEX_MPZ,
                    An, An, 0, false, true, option));
                TEST_CHECK(SPEX_vector_allocate(&vk, An, true, option));
                for (j = 0; j < An; j++)
                {
        ...
        GOTCHA;
            ...
            for (i = 0; i < An; i++)
            {
                SPEX_CHECK(SPEX_vector_realloc(L->v[i], L->v[i]->nz, option));
                SPEX_CHECK(SPEX_vector_realloc(U->v[i], U->v[i]->nz, option));
            }
	    ...
	    TEST_CHECK(SPEX_matrix_allocate(&rhos, SPEX_DENSE, SPEX_MPZ, An, 1,
	        An, false, true, option));
            for (i = 0; i < An; i++)
            {
