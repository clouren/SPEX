...
int main( int argc, char *argv[] )
{
    ...
    SPEX_TRY (SPEX_matrix_allocate(&w, SPEX_DYNAMIC_CSC, SPEX_MPZ, A->m, 1, 0, false,
        true, option));
    // reallocate w->v[0] with 1 entry
    SPEX_TRY (SPEX_vector_realloc(w->v[0], 1, option));
    // set w(0,0) = 1
    SPEX_TRY (SPEX_mpz_set_ui(w->v[0]->x[0], 1));
    w->v[0]->i[0] = 0;
    w->v[0]->nz = 1;
