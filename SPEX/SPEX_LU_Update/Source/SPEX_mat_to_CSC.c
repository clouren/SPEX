//------------------------------------------------------------------------------
// SPEX_LU_Update/SPEX_mat_to_CSC.c: convert SPEX_mat matrix to a CSC matrix
...
)
{
    ...
    else
    {
        ...
        for (i = 0 ; i < B->n ; i++)
        {
            ...
            {
                j = B->v[i]->i[p];
                A->i[ rowcount[j] ] = (perm_inv == NULL) ? i : perm_inv[i] ;
                // A->x[ rowcount[j] ] = B->v[i]->x[p]
                SPEX_CHECK(SPEX_mpz_set(SPEX_1D(A, rowcount[j], mpz),
                    B->v[i]->x[p])) ;
                rowcount[j] ++;
            }
        }
