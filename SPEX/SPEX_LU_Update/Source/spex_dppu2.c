...
)
{
    ...
    if (ks == n)
    {
        ...
        // U(k,Q(k)) and S(:,k) will be updated after calling this function, so
        // we just need to set S(2,n-1) = S(2,k)
        SPEX_CHECK(SPEX_mpq_swap(SU(n-1), SU(k)));
    }
    else // ks < n
    {
        //----------------------------------------------------------------------
        ...
        SPEX_CHECK(SPEX_mpq_mul(SL(tmp_ks),
                                SL(tmp_ks), pending_scale));
        SPEX_CHECK(SPEX_mpq_mul(SU(tmp_ks),
        ...
        //----------------------------------------------------------------------
        // swap entries in sd and S
        SPEX_CHECK(SPEX_mpz_swap(sd[tmp_ks], sd[k]));
        SPEX_CHECK(SPEX_mpq_swap(SL(tmp_ks), SL(k)));
        SPEX_CHECK(SPEX_mpq_swap(SU(tmp_ks), SU(k)));
    ...
    for (j = k; j < tmp_ks; j++)
    {
        ...
        {
            SPEX_CHECK(SPEX_mpz_sgn(&sgn, Uk_dense_row->x[Qj]));
            // skip if U(ks, Q[j]) == 0
            if (sgn == 0) { continue; }
        }
        ...
        SPEX_CHECK(spex_ipge(Uk_dense_row, h, NULL, U->v[j], Q, Q_inv,
            (const mpz_t*)sd, SU(j), j));
        // update index for last nonzero before ks-th entry
        last_nz_b4_ks = j;

        // insert new entry L(P(ks), j) to L and swap its value with U(ks, Q(j))
        SPEX_CHECK(spex_insert_new_entry(Uk_dense_row->x[Qj], L->v[j],
            SL(j), Pks, one));
        // reset U(ks, Q[j])=0
    ...
    if (ks == n)
    {
        ...
        SPEX_CHECK(SPEX_mpz_swap(U->v[n-1]->x[0], Uk_dense_row->x[Qks]));
