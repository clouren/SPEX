...
)
{
    ...
    {
        spex_chol_mex_error (1, "Usage: x = SPEX_chol_soln (A,b,option)") ;
    }
    ...
    if (nargin > 2) spex_chol_get_matlab_options (option, &mexoptions, pargin [2]) ;
    ...
    else
    {
        ...
        for (int64_t p = 0 ; p < mn ; p++)
        {
            ...
            if (status < 0)
            {
                spex_chol_mex_error (1, "error converting x to string") ;
            }
